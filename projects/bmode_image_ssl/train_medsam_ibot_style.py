...
class Args:
    ...
    name: str = "train_medsam_ibot_style"
...
def main(args: Args):
    wandb.init(project=args.project, name=args.name, config=dataclass_to_dict(args))

    if args.ckpt_dir is None: 
        args.ckpt_dir = os.environ.get("CKPT_DIR", None)
    if args.ckpt_dir is None:
        state = None 
    else: 
    ...
    # ======== Data loading ========
    train_loader, sl_train_loader, sl_val_loader = build_dataloaders(
    ...
    student = MedSAMIBot(out_dim=args.out_dim, patch_out_dim=args.patch_out_dim).to(
        args.device
    )
    teacher = MedSAMIBot(out_dim=args.out_dim, patch_out_dim=args.patch_out_dim).to(
        args.device
    )
    ...
    lr_schedule = cosine_scheduler(
        lr, args.min_lr, args.epochs, len(train_loader), args.warmup_epochs
    )
    wd_schedule = cosine_scheduler(
        args.weight_decay, args.weight_decay_end, args.epochs, len(train_loader)
    )
    momentum_schedule = cosine_scheduler(
        args.momentum_teacher, 1, args.epochs, len(train_loader)
    )
    if args.use_amp:
    ...
    for epoch in range(start_epoch, args.epochs):
        # run probing first
        run_probing(student, sl_train_loader, sl_val_loader, epoch - 1, args.device)
        ...
        if args.ckpt_dir is not None:
            ...
            torch.save(
                teacher.state_dict(), os.path.join(args.ckpt_dir, f"teacher_{epoch}.pt")
            )
        ...
        for i, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
            ...
            masks = torch.stack(masks).to(args.device)
            ...
            with torch.cuda.amp.autocast(enabled=args.use_amp):
                ...
                loss = loss_fn(student_output, teacher_output, None, masks, epoch)[
                    "loss"
                ]
...
def show_example(image_encoder, image):
    ...
    features_pca = einops.rearrange(features_pca, "(b h w) c -> b h w c", b=B, h=H, w=W)
...
def build_dataloaders(
    ...
    def ssl_transform(item):
        image = item["bmode"]
        image = torch.from_numpy(image.copy())[None, ...].repeat_interleave(3, 0)
        image1 = image.clone()
        image2 = image.clone()
    ...
    from medAI.datasets.nct2013.cohort_selection import select_cohort, apply_core_filters
    from medAI.datasets.ua_unlabeled import UAUnlabeledImages
    from medAI.datasets.nct2013 import MetadataKeys
    ...
    if include_nct:
        datasets.append(BModeDatasetV1(train_cores, transform=ssl_transform))
    ...
    # should also create the supervised loaders
    train_cores = apply_core_filters(train_cores.copy(), exclude_benign_cores_from_positive_patients=True, involvement_threshold_pct=40, undersample_benign_ratio=1)
    
    def sup_transform(item): 
        out = {}
        ...
        image -= image.min()
        image /= image.max()
        image = T.Resize((1024, 1024), T.InterpolationMode.BILINEAR)(image)
        out['bmode'] = image

        needle_mask = item["needle_mask"]
        needle_mask = torch.from_numpy(needle_mask.copy())[None, ...]
        out['needle_mask'] = needle_mask

        prostate_mask = item["prostate_mask"]
        prostate_mask = torch.from_numpy(prostate_mask.copy())[None, ...]
        out['prostate_mask'] = prostate_mask

        grade_group = item[MetadataKeys.GRADE_GROUP]
        grade_group = torch.tensor(grade_group, dtype=torch.long)
        out['grade_group'] = grade_group
    ...
    sl_train_set = BModeDatasetV1(train_cores, transform=sup_transform)
    sl_val_set = BModeDatasetV1(val_cores, transform=sup_transform)
    sl_train_loader = torch.utils.data.DataLoader(
        sl_train_set, batch_size=batch_size, shuffle=True, num_workers=8
    )
    sl_val_loader = torch.utils.data.DataLoader(
        sl_val_set, batch_size=batch_size, shuffle=True, num_workers=8
    )

    return ssl_loader, sl_train_loader, sl_val_loader
...
class MedSAMIBot(nn.Module):
    def __init__(self, out_dim=2048, patch_out_dim=2048):
        ...
        from heads_ibot import iBOTHead
...
@torch.no_grad()
def run_probing(model: MedSAMIBot, train_loader: torch.utils.data.DataLoader, val_loader, epoch, device: str):
    train_outputs = {}
    ...
    def extract_features(batch, model, device): 
        ...
        image_features = model.image_encoder_wrapped.get_image_features(bmode) # B, C, H, W

        needle_mask = batch['needle_mask'].to(device)
        needle_mask = nn.functional.interpolate(needle_mask, size=(image_features.shape[2], image_features.shape[3]), mode='nearest')
        prostate_mask = batch['prostate_mask'].to(device)
        prostate_mask = nn.functional.interpolate(prostate_mask, size=(image_features.shape[2], image_features.shape[3]), mode='nearest')

        mask = (needle_mask > 0.5) & (prostate_mask > 0.5)

        cancer = batch['cancer'].to(device)
        cancer = cancer[:, None, None, None].repeat(1, 1, image_features.shape[2], image_features.shape[3])
        cancer.shape

        from einops import rearrange
        image_features = rearrange(image_features, 'b c h w -> b h w c')
        cancer = rearrange(cancer, 'b c h w -> b h w c')
        mask = rearrange(mask, 'b c h w -> b h w c')[..., 0]

        image_features = image_features[mask]
        cancer = cancer[mask]

        return image_features, cancer

    for batch in tqdm(train_loader, desc='Linear Probing'): 
        
        image_features, cancer = extract_features(batch, model, device)
        train_outputs.setdefault('train_features', []).append(image_features)
        train_outputs.setdefault('train_labels', []).append(cancer)

    train_outputs['train_features'] = torch.cat(train_outputs['train_features'], dim=0)
    train_outputs['train_labels'] = torch.cat(train_outputs['train_labels'], dim=0)

    val_outputs = {}
    for batch in tqdm(val_loader, desc='Linear Probing'): 
        image_features, cancer = extract_features(batch, model, device)
        val_outputs.setdefault('val_features', []).append(image_features)
        val_outputs.setdefault('val_labels', []).append(cancer)

    val_outputs['val_features'] = torch.cat(val_outputs['val_features'], dim=0)
    val_outputs['val_labels'] = torch.cat(val_outputs['val_labels'], dim=0)

    from sklearn.linear_model import LogisticRegression
    from sklearn.metrics import roc_auc_score

    X_train = train_outputs['train_features'].cpu().numpy()
    y_train = train_outputs['train_labels'].cpu().numpy()
    X_val = val_outputs['val_features'].cpu().numpy()
    y_val = val_outputs['val_labels'].cpu().numpy()
    ...
    wandb.log({'train_auc_probing': train_auc, 'val_auc_probing': val_auc, 'epoch': epoch})
...
if __name__ == "__main__":
    ...
    args = Args(
        ...
        crop_scale_2=(0.8, 1),
        use_nct_data=False,
        use_ua_unlabelled_data=True, 
        use_amp=True, 
        device='cuda'
    )

    from submitit import SlurmExecutor
    executor = SlurmExecutor(folder='logs', max_num_timeout=100)
    executor.update_parameters(
        gres='gpu:a40:1', 
        mem='32G',
        time=60*16, 
        cpus_per_task=16,
        setup=['export TQDM_MININTERVAL=30', 'export WANDB_RUN_ID=${SLURM_JOB_ID}', 'export WANDB_RESUME=allow', 'export CKPT_DIR=/checkpoint/${USER}/${SLURM_JOB_ID}'],
    ...
    class Main: 
        ...
        def checkpoint(self): 
            ...
            return DelayedSubmission(Main(self.args))
