...
@hydra.main(config_path="conf", config_name="train_medsam_ssl")
def main(config):
    return SSLExperiment(config).run()
...
class SSLExperiment(Experiment):
    ...
    def show_example(self, batch):
        image = batch["bmode"].to(self.config.device)
        return self.model.show_example(image)

    def setup_model(self):
        self.model = IBotStyleModel(**self.config.model).to(self.config.device)

    def setup_optimizer(self):
        return super().setup_optimizer()

    def save_model_weights(self, score, is_best_score=False):
        if self.config.get("checkpoint_dir") is not None:
            torch.save(
                self.model.teacher.encoder.state_dict(),
                os.path.join(self.config.checkpoint_dir, f"encoder_{self.epoch}.pth"),
            )
    ...
    def forward(self, image):
        ...
        with torch.no_grad():
            ...
            if self.sinkhorn_knopp_centering_mode == "in_mask":
                ...
                target_dist = (target_token_scores / self.student_temp).softmax(-1)
                ...
                target_token_scores = self.sinkhorn(target_token_scores)
                target_dist = (target_token_scores / self.teacher_temp).softmax(-1)
...
class SimMIMModel(SelfSupervisedEncoderTrainer):
    ...
    def forward(self, image):
        ...
        for _ in range(image.shape[0]):
            masks.append(
                torch.from_numpy(self.masking_generator()).bool().to(image.device)
            )
        ...
        if False:
            ...
            projected_embeddings = einops.rearrange(
                projected_embeddings, "b c h w -> b h w c"
            )
            ...
            image_patches = einops.rearrange(
                image, "b c (h p1) (w p2) -> b h w (p1 p2 c)", p1=16, p2=16
            )
        ...
        assert (
            image_patches.shape[0] == projected_embeddings.shape[0]
    ...
    def show_example(self, image):
        ...
        projected_embeddings = (
            self.maskable_model(image, mask=masks)
            if not debug
            else self.maskable_model(image)
        )
        image_reconstruction = einops.rearrange(
            projected_embeddings, "b (p1 p2 c) h w -> b c (h p1) (w p2)", p1=16, p2=16
        )
        image_reconstruction = image_reconstruction.cpu().numpy()
        image_reconstruction = einops.rearrange(
            image_reconstruction, "b c h w -> b h w c"
        )
