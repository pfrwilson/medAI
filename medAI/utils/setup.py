...
def generate_exp_dir(project=None, group=None):
    """
...
class BasicExperimentConfig(Serializable): 
    ...
    def __post_init__(self):
        if self.exp_dir is None: 
            self.exp_dir = generate_exp_dir(self.project, self.group)
...
class BasicExperiment: 
    ...
    def setup(self): 
        if not os.path.exists(os.path.join(self.config.exp_dir, 'checkpoints')): 
            if slurm_checkpoint_dir() is not None: 
                # sym link slurm checkpoints dir to local checkpoints dir
                os.symlink(slurm_checkpoint_dir(), os.path.join(self.config.exp_dir, 'checkpoints'))
            else: 
        ...
        def excepthook(type, value, tb):
            logging.error("Uncaught exception: {0}".format(str(value)))
            import traceback
            traceback.print_tb(tb, file=open(os.path.join(self.config.exp_dir, 'out.log'), 'a'))
            logging.error(f'Exception type: {type}')
            sys.__excepthook__(type, value, tb)
        sys.excepthook = excepthook

        if self.config.resume and 'wandb_id' in os.listdir(self.config.exp_dir): 
            wandb_id = open(os.path.join(self.config.exp_dir, 'wandb_id')).read().strip()
            logging.info(f'Resuming wandb run {wandb_id}')
        else: 
            wandb_id = wandb.util.generate_id()
        ...
        # there is an issue with config class after pickling so we load it from the yaml file
        import yaml
        _config_dict = yaml.load(open(os.path.join(self.config.exp_dir, 'config.yaml')), Loader=yaml.FullLoader) 
    ...
    def submit(cls): 
        ...
        parser.add_argument('--config_path', type=str, default=None, help='Path to a config file.')
        ...
        if isinstance(cfg.cluster, LocalJobSubmissionConfig): 
            ...
            executor = submitit.AutoExecutor(folder=os.path.join(cfg.exp_dir, 'submitit_logs'))
