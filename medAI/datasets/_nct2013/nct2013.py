...
AUTOMATIC_PROSTATE_SEGMENTATION_DIR = (
...
class PatientSelector(ABC):
    @abstractmethod
    def __call__(self, split: str):
        """Returns the list of patient ids for a given split"""

    @staticmethod
    def add_arguments(parser: argparse.ArgumentParser, group=None):
        """Adds arguments to the given parser for patient selector"""
        ...
        group.add_argument(
            ...
            default="kfold",
            choices=["kfold", "loco"],
            help="The patient selector to use. Loco means 'leave one center out'",
        )

        args, _ = parser.parse_known_args()
        if args.patient_selector == "kfold":
            group.add_argument("--fold", type=int, default=0, help="The fold to use")
            group.add_argument(
                "--n-folds", type=int, default=5, help="The number of folds to use"
            )

        elif args.patient_selector == "loco":
            group.add_argument(
                "--test-center",
                type=str,
                default="UVA",
                choices=["UVA", "CRCEO", "PCC", "PMCC", "JH"],
                help="The center to leave out",
            )

    @staticmethod
    def from_argparse_args(args):
        if args.patient_selector == "kfold":
            return KFoldPatientSelector(fold=args.fold, n_folds=args.n_folds)
        elif args.patient_selector == "loco":
            return LeaveOneCenterOutPatientSelector(leave_out=args.test_center)
        else:
            raise NotImplementedError


class KFoldPatientSelector(PatientSelector):
    def __init__(self, fold: int = 0, n_folds: int = 5):
        self.fold = fold
    ...
    def __call__(self, split):
        ...
        train, val, test = get_patient_splits_by_fold(self.fold, self.n_folds)
...
class LeaveOneCenterOutPatientSelector:
    def __init__(self, leave_out="UVA"):
        self.leave_out = leave_out

    def __call__(self, split):
        if split == "test":
            logging.info(f"Using leave one center out with {self.leave_out}")
        if split not in ["train", "val", "test", "all"]:
            raise ValueError(
                f"Split must be 'train', 'val', 'test' or 'all, but got {split}"
            )

        train, val, test = get_patient_splits_by_center(self.leave_out)
...
class CoresSelector:
    def __init__(
        self,
        patient_selector: PatientSelector = KFoldPatientSelector(fold=0, n_folds=5),
        ...
        self.min_involvement = min_involvement
        self.remove_benign_from_positive_patients = remove_benign_from_positive_patients
        self.benign_to_cancer_ratio = benign_to_cancer_ratio
        self.seed = seed

    def __call__(self, split: str):
        """Returns the list of core ids for a given split"""
        patient_ids = self.patient_selector(split)
        core_ids = get_core_ids(patient_ids)
        if self.remove_benign_from_positive_patients:
            core_ids = remove_benign_cores_from_positive_patients(core_ids)
        if self.min_involvement is not None:
            core_ids = remove_cores_below_threshold_involvement(
                core_ids, self.min_involvement
            )
        if self.benign_to_cancer_ratio is not None:
            core_ids = undersample_benign(
                core_ids,
                self.seed,
                self.benign_to_cancer_ratio,
            )
...
def _core_id_to_tag(id):
    """
    Returns the integer core id for the given tag.

    Tags look like "CRCEO-0001_LML", core id is the
    unique integer id for the core which was assigned
    arbitrarily.
    """

    return CORE[CORE.id == id].tag.values[0]


def get_prostate_mask(masks_dir, core_id):
    """Loads the prostate mask for the given core id.

    The convention is to save the prostate mask as a png file with the same name as the core "tag".

    Args:
        masks_dir (str): The directory containing the prostate masks.
        core_id (int): The core id.

    """

    tag = _core_id_to_tag(core_id)
    image = Image.open(os.path.join(masks_dir, f"{tag}.png"))
...
def prostate_mask_available(masks_dir, core_id):
    """Returns True if the prostate mask is available for the given core id.
    ...
    tag = _core_id_to_tag(core_id)
    return f"{tag}.png" in os.listdir(masks_dir)
...
class ExactNCT2013BModeImages(ExactNCT2013Cores):
    """Dataset for the B-mode images from the NCT 2013 dataset.

    Provides access to the downsampled B-mode images, stored in compressed resolution and format
    characteristic of what you would see in a saved cineloop file, i.e. not full RF resolution.
    additionally, provides access to the needle mask, prostate mask, and other metadata available with the
    dataset optionally the prostate mask.
    Instantiating this class and accessing items using square brackets will return a dictionary with the following keys:
        - id: the unique integer id for the core
        - tag: the tag for the core, e.g. "CRCEO-0001_LML"
        - grade: the grade of the core, "Benign", "GS7", "GS8", "GS9", "GS10"
        - psa: the PSA of the patient
        - age: the age of the patient
        - pct_cancer: the percentage of the core that is cancerous
        - loc: the anatomical location of the core
        - bmode: the B-mode image, a numpy array of variable size based on the core. The image
            will have dimensions H, W representing the axial and lateral dimensions of the core. Higher axial
            coordinates are further from the transducer and higher lateral coordinates are closer to the sonographer's hand
        - needle_mask: the needle mask, a numpy array

    Args:
        split (str, optional): The split to use, one of "train", "val", "test", "all". Defaults to "train".
        transform (callable, optional): A function/transform that takes in a dictionary and returns a transformed version. Defaults to None.
        cohort_selection_options (CohortSelectionOptions, optional): Options for selecting the cohort. Defaults to CohortSelectionOptions().
        prostate_mask_dir (str, optional): The directory containing the prostate masks. Defaults to AUTOMATIC_PROSTATE_SEGMENTATION_DIR.
    ...
    def __init__(
        ...
        transform=None,
        cores_selector: CoresSelector = CoresSelector(),
        prostate_mask_dir=AUTOMATIC_PROSTATE_SEGMENTATION_DIR,
    ) -> None:
        super().__init__(split=split, cores_selector=cores_selector)
        self.transform = transform

        # load the needle mask
        self.needle_mask = np.load(
            os.path.join(BMODE_DATA_PATH, "needle_mask.npy"), mmap_mode="r"
        )

        # Load the B-mode data and indexings
        ...
        self._prostate_mask_dir = prostate_mask_dir
        if prostate_mask_dir is not None:
            available_masks = [
                id
                for id in self.core_info.id.values
                if prostate_mask_available(prostate_mask_dir, id)
            ]
    ...
    def __getitem__(self, index: int) -> tp.Tuple[tp.Any, tp.Any]:
        ...
        if self._prostate_mask_dir is not None:
            out["prostate_mask"] = get_prostate_mask(self._prostate_mask_dir, out["id"])
...
class ExactNCT2013RFImages(ExactNCT2013Cores):
    """Dataset for the rf images from the NCT 2013 dataset.

    Provides access to the downsampled rf images preprocessed from the original IQ data.
    additionally, provides access to the needle mask, prostate mask, and other metadata available with the
    dataset optionally the prostate mask.
    Instantiating this class and accessing items using square brackets will return a dictionary with the following keys:
        - id: the unique integer id for the core
        ...
        - pct_cancer: the percentage of the core that is cancerous
        - loc: the anatomical location of the core
        - rf_image: the rf image, a numpy array of variable size based on the core. The image
            will have dimensions H, W representing the axial and lateral dimensions of the core. Higher axial
            coordinates are further from the transducer and higher lateral coordinates are closer to the sonographer's hand
        - needle_mask: the needle mask, a numpy array
    ...
    Args:
        ...
        cohort_selection_options (CohortSelectionOptions, optional): Options for selecting the cohort. Defaults to CohortSelectionOptions().
        prostate_mask_dir (str, optional): The directory containing the prostate masks. Defaults to AUTOMATIC_PROSTATE_SEGMENTATION_DIR.
        cache (bool, optional): Whether to cache the data in memory. Defaults to False.
    ...
    def __getitem__(self, index):
        if self.cache and index in self._cache:
            ...
            if self._prostate_mask_dir is not None:
                out["prostate_mask"] = get_prostate_mask(
                    self._prostate_mask_dir, out["id"]
                )
...
class _ExactNCT2013DatasetWithProstateSegmentation(ABC):
    def __init__(self, dataset: ExactNCT2013Cores, transform=None):
        warn(
            "This class is deprecated. Use ExactNCT2013BmodeImages or ExactNCT2013RFImages instead.",
            DeprecationWarning,
        )
        ...
        available_masks = [
            id
            for id in self.dataset.core_info.id.values
            if self.prostate_mask_available(id)
        ]
...
class _ExactNCTPatchesDataset(Dataset):
    def __init__(
        self,
        dataset: _ExactNCT2013DatasetWithAutomaticProstateSegmentation,
