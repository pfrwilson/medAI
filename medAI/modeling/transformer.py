...
class PositionalEmbeddings2DWithBilinearInterpolation(torch.nn.Module): 
    def __init__(self, spatial_size, embeddings_grid_size, n_features, spatial_origin=(0, 0)): 
        super().__init__()
        self.spatial_size = spatial_size
        self.spatial_origin = spatial_origin
        self.n_features = n_features

        self.embeddings = torch.nn.Parameter(torch.randn(embeddings_grid_size[0], embeddings_grid_size[1], n_features, dtype=torch.float32))
    ...
    def forward(self, positions): 
        ...
        x = x / self.spatial_size[0] * self.embeddings.shape[0] # position in embeddings grid
        y = y / self.spatial_size[1] * self.embeddings.shape[1] # position in embeddings grid
...
class MultiHeadAttentionWith2DRelativePositionalEmbeddings(nn.Module):
    def __init__(
        self, d_model: int, n_heads: int, d_k: int | None = None, d_v: int | None = None, 
        max_spatial_distance=(10, 10), embeddings_grid_shape=(10, 10),
    ):
        """
        Implements multi-head scaled dot product self attention.
        Args:
        ...
        self.embeddings = PositionalEmbeddings2DWithBilinearInterpolation(
            (spatial_size_x, spatial_size_y), embeddings_grid_shape, n_heads, spatial_origin=spatial_origin
        )    
    ...
    def compute_positional_embedding_coefficients(self, positions): 
        ...
        positions = einops.repeat(
            positions, 
            'b n1 d -> b n1 n2 d', n1=N, n2=N
        ) 
        positions2 = einops.rearrange(
            positions, 'b n1 n2 d -> b n2 n1 d'
        )
...
class TransformerEncoderWith2DRelativeEmbeddings(nn.Module):
    ...
    def forward(self, X, positions, mask=None, return_attn=False):
        ...
        for i in range(self.n_layers):
            if return_attn:
                attn_layer_out, attn = self.attn_blocks[i](X, positions, mask=mask, return_attn=True)
            else:
